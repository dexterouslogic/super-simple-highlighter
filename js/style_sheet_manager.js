  ...
  init() {
		...
		const rules = [
				`.${this.sharedHighlightClassName} .${StyleSheetManager.CLASS_NAME.CLOSE} {
						${StyleSheetManager.DIRECTIVES.CLOSE}
				}`,
				...
				.${this.sharedHighlightClassName} .${StyleSheetManager.CLASS_NAME.CLOSE}:focus {
						${StyleSheetManager.DIRECTIVES.CLOSE_HOVER_FOCUS}
				}`    
		]
		...
		return new ChromeStorage().get(ChromeStorage.KEYS.HIGHLIGHT_BACKGROUND_ALPHA).then(a => {
			...
			const rule = `.${highlightDefinition.className}
				${JSON.stringify(rules, null, '\t')
					...
					.replace(/\}/g, ';}')}
			`
			const elm = this.document.getElementById(this.styleElementId)
...
StyleSheetManager.DIRECTIVES = {
  ...
  cursor: pointer;
  z-index: 1000;
	transition: opacity 275ms, transform 275ms cubic-bezier(0.175, 0.885, 0.32, 1.275);
	padding: 0;
	border: none;
